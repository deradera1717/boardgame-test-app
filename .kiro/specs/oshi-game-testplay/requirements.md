...
1. WHEN 労働フェーズが開始される THEN System SHALL 6種類の報酬配分カードを表示する
2. WHEN プレイヤーが報酬配分カードを選択する THEN System SHALL 選択を記録し他プレイヤーの選択を待つ
3. WHEN 全プレイヤーがカードを選択完了する THEN System SHALL サイコロを振り結果を表示する
4. WHEN サイコロの結果が確定する THEN System SHALL 各プレイヤーの選択カードに基づいて資金を配布する
...
2. WHEN プレイヤーが選択を行う THEN System SHALL 他プレイヤーに選択内容を隠して記録する
3. WHEN 全プレイヤーが選択完了する THEN System SHALL 全員の選択を同時に公開する
4. WHEN プレイヤーが「休む」を選択する THEN System SHALL 労働フェーズで得た資金と同額を追加付与する
5. WHEN プレイヤーが「推しかつする」を選択する THEN System SHALL そのプレイヤーを推し活フェーズに参加させる

### 要件5

**ユーザーストーリー:** プレイヤーとして、ファンサスポットを予測したいので、ファンサスポットカードの情報を確認してオタクコマを配置したい。

#### 受入基準

1. WHEN 推し活フェーズが開始される THEN System SHALL ファンサスポットカード3枚をランダムに公開する
2. WHEN ファンサスポットカードが公開される THEN System SHALL カードの表裏と向きをランダムに決定する
3. WHEN プレイヤーがグッズを購入する THEN System SHALL 資金を消費してグッズチップを付与する
4. WHEN プレイヤーがオタクコマを配置する THEN System SHALL 花道ボードの指定マスにコマを配置する
5. WHEN 1つのマスに3個のオタクコマが配置される THEN System SHALL それ以上の配置を制限する
...
1. WHEN ファンサタイムが開始される THEN System SHALL 各推し（A、B、C）のファンサスポットをサイコロで決定する
2. WHEN 推しがオタクコマの目の前に停止する THEN System SHALL そのマスのオタクコマで6ポイントを山分けする
3. WHEN うちわを持つオタクコマの隣に推しが停止する THEN System SHALL そのオタクコマに1ポイントを付与する
4. WHEN ペンライトを持つオタクコマの向かい側に推しが停止する THEN System SHALL そのオタクコマに1ポイントを付与する
5. WHEN 差し入れを持つオタクコマの目の前に推しが停止する THEN System SHALL そのオタクコマのポイントを2倍にする
